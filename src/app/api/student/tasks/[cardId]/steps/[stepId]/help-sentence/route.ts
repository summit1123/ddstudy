...
export async function POST(
  ...
  const log = await addStudentLog({
    ...
    cardId,
    stepId,
    eventType: "help_sentence_viewed",
    payloadJson: defaultLogPayload({ helpSentence: validation.step.helpSentence }),
  });
  const { summary } = await buildTaskSummary(studentId, cardId);
  return NextResponse.json({ log, summary, helpSentence: validation.step.helpSentence });
}
