...
export async function buildStudentReport(studentId: string, cardId?: string) {
  ...
  const stuckSteps = perStep.filter(
    (item) =>
      item.confusedCount > 0 ||
      item.simplifyCount > 0 ||
      item.helpSentenceViewedCount > 0 ||
      (item.quizAnswered && !item.isCorrect)
  );
  const firstStuckStep = stuckSteps[0];
  const firstIncompleteStep = perStep.find((item) => !item.isCompleted);
  ...
  for (const item of perStep) {
    ...
    if (item.helpSentenceViewedCount > 0) difficultyTags.add("도움 요청");
    if (item.quizAnswered && !item.isCorrect) difficultyTags.add("반복 확인");
  ...
  const recommendationByTag: Record<string, string> = {
    ...
    "도움 요청": "학생이 실제로 말할 수 있는 도움 요청 문장을 먼저 읽고, 같은 문장으로 질문하게 해보세요.",
  ...
  if (firstStuckStep) {
    recommendations.unshift(
      `${firstStuckStep.step.order}단계에서 먼저 멈춤 신호가 보였습니다. 이 단계의 행동을 한 문장으로 다시 말하게 하고, 학생 답을 확인한 뒤 다음 단계로 넘겨 주세요.`
    );
  } else if (firstIncompleteStep) {
    recommendations.unshift(
      `${firstIncompleteStep.step.order}단계부터 수행 기록이 없습니다. 학생 화면에서 첫 행동을 함께 시작하고, 완료 버튼과 답 입력까지 진행되는지 확인해 주세요.`
    );
  }
  const stuckStepText = firstStuckStep
    ? `${firstStuckStep.step.order}단계 '${firstStuckStep.step.stepText}'`
    : "";
  ...
  const report: Report = {
    ...
    summary: `${student.nickname}는 완료율 ${summary.completionRate}%로 과제를 수행했고, 도움 요청은 ${summary.helpRequestCount}회였습니다.${
      stuckStepText
        ? ` 가장 먼저 확인할 지점은 ${stuckStepText}입니다.`
        : firstIncompleteStep
          ? " 아직 도움 요청이나 오답 로그는 없고, 수행 시작 여부를 먼저 확인해야 합니다."
          : " 현재 뚜렷한 막힘 신호는 없습니다."
    }`,
    ...
    parentMemo:
      ...
      (stuckStepText ? `가장 먼저 막힌 곳은 ${stuckStepText}입니다. ` : "") +
